...
def generate(
  ...
  with torch.no_grad():
    ...
    if input_images:
      with hotline.annotate('Input Images'):
      ...
      with hotline.annotate('Input Noise'):
        latents = torch.randn(noise_shape, generator=generator, device=device)
        latents *= sampler.initial_scale

    with hotline.annotate('Load Stable Diffusion Weights'):
      diffusion = models.get('diffusion') or model_loader.load_diffusion(device)
      with hotline.annotate('aten::to'):
        diffusion.to(device)

    with hotline.annotate('Inference'):
      timesteps = tqdm(sampler.timesteps)
      for i, timestep in enumerate(timesteps):
        with hotline.annotate(f'Inference Step {i}'):
          with hotline.annotate('aten::repeat'):
          ...
          with hotline.annotate('aten::add_'):
            if do_cfg:
    ...
    with hotline.annotate('Load Decoder Weights'):
      decoder = models.get('decoder') or model_loader.load_decoder(device)
      with hotline.annotate('aten::to'):
        decoder.to(device)

    with hotline.annotate('Decode'):
      images = decoder(latents)
      to_idle(decoder)
      del decoder

    with hotline.annotate('Postprocess'):
